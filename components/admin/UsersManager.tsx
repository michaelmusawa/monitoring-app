    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600">Add, edit or remove application users. Changes are posted to example admin endpoints.</p>
        </div>
      ...
      <div className="p-4 border rounded bg-white">
        <form className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end" onSubmit={addUser}>
          ...
          <div className="md:col-span-1">
            ...
            <Input value={form.email} onChange={(e) => setForm((s) => ({ ...s, email: e.target.value }))} placeholder="email@domain" />
          </div>
              ...
              <tr key={u.id} className="border-t">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <button className="px-2 py-1 text-xs border rounded" onClick={() => startEdit(u)}>
                      Edit
                    </button>
                    <button
...
export function ChecklistEditor(): JSX.Element {
  ...
  const [categories, setCategories] = useState<Category[]>([
    { id: uid("cat-"), name: "Mobilization", tasks: [{ id: uid("t-"), label: "Contract signing" }] },
  ]);
  ...
  function addTask(catId: string) {
    setCategories((s) => s.map((c) => (c.id === catId ? { ...c, tasks: [...c.tasks, { id: uid("t-"), label: "New task" }] } : c)));
  }
  ...
  async function saveTemplates() {
    ...
    setMessage(null);
    try {
      const payload = categories.map((c) => ({ id: c.id, name: c.name, tasks: c.tasks }));
  ...
  return (
    <div className="space-y-4">
          ...
          <div key={cat.id} className="border rounded p-3 bg-white">
            <div className="flex items-center justify-between mb-2">
              <input
                ...
                value={cat.name}
                onChange={(e) => setCategories((s) => s.map((c) => (c.id === cat.id ? { ...c, name: e.target.value } : c)))}
              />
              <div className="flex items-center gap-2">
              ...
              {cat.tasks.map((t) => (
                <li key={t.id} className="flex items-center gap-2">
                  <input
                    ...
                    value={t.label}
                    onChange={(e) => setCategories((s) => s.map((c) => (c.id === cat.id ? { ...c, tasks: c.tasks.map((x) => (x.id === t.id ? { ...x, label: e.target.value } : x)) } : c)))}
                  />
                  <button className="px-2 py-1 text-xs border rounded text-rose-600" onClick={() => removeTask(cat.id, t.id)}>
                    Remove
                  </button>
                </li>
              ))}
              {cat.tasks.length === 0 && <li className="text-sm text-gray-500">No tasks yet</li>}
            </ul>
          </div>
        ))}
      </div>
      ...
      <div className="flex gap-2 items-center">
        <input className="border px-2 py-1 rounded flex-1" placeholder="New category name" value={newCategoryName} onChange={(e) => setNewCategoryName(e.target.value)} />
